...
- `apps/kimi-desktop`: the Electron desktop app (Codex-style UI, React renderer, attach-mode connection discovery). Data layer lives in `src/renderer/src/lib/` (REST `ApiClient` over `@moonshot-ai/protocol`, dual WS clients, transcript sync over `@moonshot-ai/transcript`); UI components in `src/renderer/src/components/` (sidebar / chat timeline / markdown pipeline / composer). The main process scans the kap-server instance registry (newest heartbeat) and exposes `window.kimiDesktop.getConnection()` via preload.
