/**
 * Per-(session, agent) transcript state for the chat view — a thin observable
 * wrapper over the package's L1 convergence path (`applyOperation` on an
 * `AgentState`). The reducer is NOT re-implemented here; `@moonshot-ai/transcript`
 * owns the merge rules. State arrives through exactly two channels:
 *
 ...
 *
 * `onGap` surfaces `append` placement gaps so the sync layer can trigger a
 * catch-up / full refresh (the WS channel carries no snapshots to fall back
 * on).
 */
...
export class TranscriptChatStore {
  ...
  /** Called when an `append` op could not be placed — the sync layer should catch up. */
