...
import {
  ...
  viewFacingToLR,
} from '#/office/vendored/scene/systems/movementFacing';
import { SpineCharacter } from '#/office/vendored/scene/characters/SpineCharacter';
import { isSpineReady } from '#/office/vendored/scene/assets/loadSpineAssets';
import { Bubble } from '#/office/vendored/scene/ui/Bubble';
import { StatusLabel } from '#/office/vendored/scene/ui/StatusLabel';
...
export class AgentEntity extends Container {
  ...
  apply(patch: Partial<Agent>) {
    ...
    if (patch.name !== undefined && patch.name !== prevName) {
      this.statusLabel.setName(patch.name);
    }

    if (this.useSpine && this.spineChar) {
      if (
        patch.viewFacing !== undefined &&
        patch.viewFacing !== prevViewFacing
      ) {
        this.spineChar.setViewFacing(patch.viewFacing);
      }
      if (patch.facing !== undefined && patch.facing !== prevFacing) {
        this.spineChar.setFacing(patch.facing);
      }
      if (
        (patch.state !== undefined && patch.state !== prevState) ||
        patch.customAnimation !== prevCustomAnimation
      ) {
        this.spineChar.playState(this.agent.state, this.agent.customAnimation);
      }
      if (patch.color !== undefined && patch.color !== prevColor) {
  ...
  updateVisuals(state: AgentState, dt: number) {
    if (this.useSpine && this.spineChar) {
      if (
        state === 'walking' &&
        this.agent.targetX !== undefined &&
        this.agent.targetY !== undefined
      ) {
